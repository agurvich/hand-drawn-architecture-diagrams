...
| `getMergeIndex` | `src/client/mergeIndex.ts` | The derivation as a `computed` keyed on the Editor; holds ids, flags, counts and actor sets, never coordinates |
...
| stroke corpus | `src/shared/sketch/__fixtures__/strokes/` | 45 recorded strokes with expected verdicts; each names its `via` — agent-drawn or pencil |
| recorded corpus | `src/shared/sketch/__corpus__/` | The 276-stroke iPad drawing, decoded and labelled; `corpus.test.ts` scores the classifier against it |
| `shouldConnect` | `src/client/sketch/convertPolicy.ts` | When a stroke becomes a connection, as a decision replayable without an editor |
