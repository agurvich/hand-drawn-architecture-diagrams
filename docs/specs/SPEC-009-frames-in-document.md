...
- Exporting frames deterministically, and **never emitting a document the validator would reject**
...
- **Downgrading.** There is no v2 → v1 export. And the honest limit: a **v1 build handed a v2
  document** cannot be improved by this spec — it will report `document.frames: unknown key`, because
...
- **Fixing frame ordering across clients.** Frames carry a fractional `index` on the record
  (SPEC-008); two clients creating frames concurrently can produce a tie. This spec makes *export*
  deterministic in the presence of one (FR-003) and does not otherwise touch how indices are minted.
- **Which frame you are viewing.** Session state SPEC-008 keeps out of document scope, and not part
  of a shared artifact. A frame you were viewing that an import removes is SPEC-008 FR-004's
  deleted-frame case, which already drops the viewer out.
...
A frame in a document carries what SPEC-008's record carries, minus the machinery: a name, a note,
which nodes read as folded, and what is highlighted.

**Frame ids are their own namespace**, deliberately not shared with nodes and connections. The
node/connection namespace is shared because both mint `shape:<id>`, so a collision means one record
overwriting another (`document.ts` says exactly this). A frame mints `diagramFrame:<id>` — a
different record type in a different id space — so a frame named `auth` beside a node named `auth`
...
- [ ] A frame id must match `DOCUMENT_ID_PATTERN` and be unique **among frames**; a frame id equal
...
- [ ] Each of these is rejected with a message naming its path, each with its own test: a missing or
      ...
      non-array `highlighted`; a non-string entry in it; an unknown key on a frame
- [ ] **Two distinct errors for a bad `collapsed` key**, because they are two different authoring
      mistakes: `frames[0].collapsed["x"]: no node with id "x"` when nothing has that id, and
      `frames[0].collapsed["x"]: names a connection, which cannot be collapsed` when it names one.
...
- [ ] Frame order is the array's order. The record's fractional `index` is generated on import, and
...
exists to hold and the one frames most easily break. A frame can legitimately name things the
document cannot carry: SPEC-008 deliberately keeps *stale* frames rather than deleting them, and
...
- [ ] Export emits every frame in the room, with node and connection ids stripped of the `shape:`
...
- [ ] **One undo restores the frames too.** The import's frame writes — both the removal of the old
...
- [ ] The guide documents every frame field and no field the schema lacks, and states that frames are
      **replaced** on import
- [ ] It explains what a frame is **for** — a diagram plus a sequence of frames is a walkthrough, and
      the frames worth writing are the ones that change what is folded
- [ ] It says a frame is a **lens**: stepping never changes the diagram, so an author should not
      expect a frame to "set" anything
- [ ] It carries a worked example with frames, imported by the existing extraction test
...
export interface DocumentFrame {
  ...
  note?: string
  /** Node ids that read as folded (or explicitly open) while this frame is active. */
  collapsed?: Record<string, boolean>
...
/**
 * A v1 document, as v2: the same document plus an empty frames array.
 *
...
//
// That reorder is what lets a v1 document carrying `frames` be rejected by
...
// exportDocument reads the room's frame records and passes them to toDocument,
...
//
// importDocument removes every frame record and writes the document's, both
// inside its EXISTING editor.run() and after its EXISTING history mark, as
// direct store writes. Not through frameView.ts's mutations: those are
...
src/
├── shared/
│   ├── document.ts                 # version 2, frames, the upgrade, the reorder
...
└── client/
    └── documentIO.ts               # frames exported and replaced, in one recorded change
docs/
...
### Phase 3: The frames key
- `DocumentFrame`, its validation, the two `collapsed` errors, the frame-only id namespace
- `toDocument`/`fromDocument` carrying frames, references filtered through `documentable`, indices
  generated from array position, export sorted by `(index, id)`

### Phase 4: Export, import, the guide and proof
- `documentIO.ts`: frames exported; frames replaced inside the existing recorded change
- The guide's frames section and worked example; the narrowed deferred-key guard; the version sweep
